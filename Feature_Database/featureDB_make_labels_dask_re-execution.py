...
chunkshape_folder = sys.argv[1].strip("()").replace(" ", "").replace(",", "_")
#
file_paths = sorted(glob.glob(f'/shared/dask_chunks_{chunkshape_folder}' + '/imerg2022/3B-HHR*.HDF5.carved'))
...
for i in range(0, len(data)):
	dask_data = da.from_array(data[i][()], chunks=dask_array_chunkshape)
	c = dask_data >= thresh
	mask = c.compute()
