...
def pf_search(flist: list[str], dt_str: list[str], dyamond_mask_file: str, use_connectivity: int, dyamond_ccl_initial_file: str, dyamond_ccl_file: str, hidden_path: str, min_voxels: int, just_48: bool) -> None:
    ...
    if make_mask:
        ...
        for midx, mfile in enumerate(loop_src):
            ...
            datapath = mfile + '/PRECTOT'
            dask_pr = da.from_array(_pr, chunks=dask_array_chunkshape, name=datapath)
            c = dask_pr >= pr_floor
            c.datapath = datapath
            dask_pr = c.compute()
...
if __name__ == '__main__':
    ...
    base_data_path = [f'/shared/dask_selections_{chunkshape_input}/POMD/discover/'][0]
    dir_list = ("202001", "202002")
    hidden_path = [f'/shared/dask_selections_{chunkshape_input}/POMD/output/'][0]
