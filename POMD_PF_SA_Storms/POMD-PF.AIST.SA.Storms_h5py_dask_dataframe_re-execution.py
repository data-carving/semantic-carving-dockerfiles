...
def bins_dfs(df1):
    ...
    for i in range(len(keys)):
        if (keys[i] == 'filename' or keys[i] == 'partition_number'):
            continue
        ...
        if min <= 0.0:
            if keys[i] == "LON" or keys[i] == "R_LON":
                ...
                dict_bins[keys[i]] = bins
                dict_log[keys[i]] = False
            elif keys[i] == "RAINGRID_5" or keys[i] == "VOLRAINGRID_5" or keys[i] == "RAINAREA_5" or keys[
                i] == "VOLRAIN_5":
                # log bins
                min1 = 0.0
...
dict_IMERG = {}
IMERG_hf = h5py.File(f'/shared/dask_partitions_{npartitions}/POMD/IMERG-PF/level2/202002.h5.carved','r')
dataset_names = list(IMERG_hf.keys())

datasets_to_include = ["RAINAREA","MEANRAINRATE","VOLRAIN","RAINAREA_5","VOLRAIN_5","LON","LAT","R_ORIENTATION", "R_SOLID"]

for dataset in dataset_names:
    if dataset in datasets_to_include:
        dict_IMERG[dataset] = IMERG_hf[dataset][()].astype('<f8')

#read GEOS5 dataset
# file_GEOS5_l2 = '/shared/POMD-PF/GEOS5-PF/level2/202002.HDF'
# dict_GEOS5=read_sds(file_GEOS5_l2)
dict_GEOS5 = {}
GEOS5_hf = h5py.File(f'/shared/dask_partitions_{npartitions}/POMD/GEOS5-PF/level2/202002.h5.carved','r')
dataset_names = list(GEOS5_hf.keys())

datasets_to_include = ["RAINAREA","MEANRAINRATE","VOLRAIN","RAINAREA_5","VOLRAIN_5","LON","LAT","R_ORIENTATION", "R_SOLID"]

for dataset in dataset_names:
    if dataset in datasets_to_include:
        dict_GEOS5[dataset] = GEOS5_hf[dataset][()].astype('<f8')
...
df_IMERG_dask = dd.from_pandas(df_IMERG, npartitions=npartitions)
df_GEOS5_dask = dd.from_pandas(df_GEOS5, npartitions=npartitions)

# import interposed_dask_dataframe as dd

# df_IMERG_dask = dd.from_pandas(df_IMERG, npartitions=npartitions, filename="/shared/POMD/IMERG-PF/level2/202002.h5")
# df_GEOS5_dask = dd.from_pandas(df_GEOS5, npartitions=npartitions, filename="/shared/POMD/GEOS5-PF/level2/202002.h5")
...
df_IMERG_SA_ST = df_IMERG_dask.loc[
    (df_IMERG_dask['LON'] > -85.0) & (df_IMERG_dask['LON'] < -30.0) & (df_IMERG_dask['LAT'] > -60.0) & (df_IMERG_dask['LAT'] < 15.0) & (
                df_IMERG_dask['RAINAREA'] > 2000.0) & (df_IMERG_dask['RAINAREA_5'] > 100.0)]
df_GEOS5_SA_ST = df_GEOS5_dask.loc[
    (df_GEOS5_dask['LON'] > -85.0) & (df_GEOS5_dask['LON'] < -30.0) & (df_GEOS5_dask['LAT'] > -60.0) & (df_GEOS5_dask['LAT'] < 15.0) & (
                df_GEOS5_dask['RAINAREA'] > 2000.0) & (df_GEOS5_dask['RAINAREA_5'] > 100.0)]
...
compare_dfs(df_IMERG_SA_ST.compute(), df_GEOS5_SA_ST.compute(), 'IMERG', 'GEOS5')
